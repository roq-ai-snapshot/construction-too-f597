...
import { deleteOutletById } from 'apiSdk/outlets';
import { deleteToolById } from 'apiSdk/tools';
...
function CompanyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CompanyInterface>(
    ...
    () =>
      getCompanyById(id, {
        relations: ['user', 'outlet', 'tool'],
      }),
  );

  const outletHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
  ...
  const toolHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('outlet', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/outlets/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.outlet?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/outlets/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>
              ...
              {hasAccess('tool', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/tools/create?company_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.tool?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/tools/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
                            <Td>
