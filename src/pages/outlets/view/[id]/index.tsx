...
import { deleteRentalById } from 'apiSdk/rentals';
...
function OutletViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OutletInterface>(
    ...
    () =>
      getOutletById(id, {
        relations: ['company', 'rental'],
      }),
  );

  const rentalHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('outlet', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/outlets/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('rental', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/rentals/create?outlet_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.rental?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/rentals/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.rent_date as unknown as string}</Td>
                            <Td>{record.return_date as unknown as string}</Td>
                            <Td>
